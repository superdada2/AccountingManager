...
export async function loadInvoice(req, res) {
  ...
  const Class = await class_enum.findAll().map(i => {
    return i.dataValues
  })
  const country = await country_enum.findAll().map(i => {
    return i.dataValues
  })
  const currency = await currency_enum.findAll().map(i => {
    return i.dataValues
  })
  const product = await product_enum.findAll().map(i => {
    return i.dataValues
  })
  const revenue_type = await revenue_type_enum.findAll().map(i => {
    return i.dataValues
  })
  const status = await status_enum.findAll().map(i => {
    return i.dataValues
  })
  const subscription = await subscription_enum.findAll().map(i => {
    return i.dataValues
  })
  const type = await type_enum.findAll().map(i => {
    return i.dataValues
  })
  csv()
    .fromFile(path)
...
async function loadIfNotExist(sequelizeInterface, data) {
  data.forEach(async i => {
    const exist = await sequelizeInterface.findOne({
      where: {
        data: i
      }
    })
